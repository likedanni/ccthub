...
--
-- Host: 127.0.0.1    Database: cct-hub
-- ------------------------------------------------------
...
SET @@GLOBAL.GTID_PURGED=/*!80000 '+'*/ 'ae569326-c43f-11f0-8538-99a77b71c589:1-326620';
...
CREATE TABLE `activities` (
  ...
  `merchant_id` bigint DEFAULT NULL COMMENT '商户ID，为空则表示平台活动',
  ...
  `participation_limit` int DEFAULT NULL COMMENT '参与人数限制',
  `requirement_type` tinyint DEFAULT '0' COMMENT '参与条件: 0-无, 1-需购票, 2-需积分, 3-需报名',
  `requirement_value` varchar(200) DEFAULT NULL COMMENT '条件值',
  ...
  KEY `idx_type` (`type`) COMMENT '活动类型索引',
  KEY `idx_time_range` (`starts_at`,`ends_at`) COMMENT '时间范围索引',
  KEY `idx_status` (`status`) COMMENT '活动状态索引',
  KEY `idx_audit_status` (`audit_status`) COMMENT '审核状态索引',
  CONSTRAINT `fk_activities_merchant` FOREIGN KEY (`merchant_id`) REFERENCES `merchants` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='营销活动主表，支持平台和商户活动。参与记录见 activity_participations (下批)。';
...
CREATE TABLE `activity_participations` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '参与记录ID，主键自增',
  ...
  `reward_detail` json DEFAULT NULL COMMENT '实际获得的奖励详情，JSON格式',
  `checkpoints_progress` json DEFAULT NULL COMMENT '打卡点完成进度，JSON格式',
  `joined_at` datetime DEFAULT (now()) COMMENT '参与时间',
  `completed_at` datetime DEFAULT NULL COMMENT '完成时间',
  `updated_at` datetime DEFAULT (now()) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_activity_user_unique` (`activity_id`,`user_id`) COMMENT '活动用户唯一索引',
  KEY `idx_activity_user` (`activity_id`,`user_id`) COMMENT '活动用户联合索引',
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_participation_status` (`participation_status`) COMMENT '参与状态索引',
  KEY `idx_joined_at` (`joined_at`) COMMENT '参与时间索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户参与营销活动的记录表。用于跟踪进度和发放奖励。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `articles` (
  ...
  KEY `idx_published_at` (`published_at`) COMMENT '发布时间索引',
  KEY `idx_is_pinned` (`is_pinned`) COMMENT '置顶索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='平台资讯、攻略、公告等文章内容表。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `coupons` (
  ...
  `min_spend` decimal(10,2) DEFAULT NULL COMMENT '最低消费金额门槛',
  `applicable_type` tinyint NOT NULL COMMENT '适用范围: 1-全平台, 2-指定商户, 3-指定商品',
  `applicable_ids` json DEFAULT NULL COMMENT '适用的商户或商品ID列表，JSON数组格式',
  `validity_type` tinyint NOT NULL COMMENT '有效期类型: 1-固定时段, 2-领取后生效',
  `valid_days` int DEFAULT NULL COMMENT '领取后有效天数（当validity_type=2时）',
  `starts_at` datetime DEFAULT NULL COMMENT '有效期开始时间',
  `expires_at` datetime DEFAULT NULL COMMENT '有效期结束时间',
  `total_quantity` int DEFAULT NULL COMMENT '发放总张数，null表示不限量',
  `remaining_quantity` int DEFAULT NULL COMMENT '剩余可发放张数',
  `limit_per_user` int DEFAULT '1' COMMENT '每人限领张数',
  `status` tinyint DEFAULT '0' COMMENT '状态: 0-未开始, 1-发放中, 2-已结束, 3-停用',
  `created_at` datetime DEFAULT (now()) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_status` (`status`) COMMENT '状态索引',
  KEY `idx_applicable_type` (`applicable_type`) COMMENT '适用范围索引',
  KEY `idx_expires_at` (`expires_at`) COMMENT '过期时间索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='优惠券定义模板。用户领取后生成 user_coupons 记录。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `data_dictionaries`
--
...
CREATE TABLE `data_dictionaries` (
  ...
  `updated_at` datetime DEFAULT (now()) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_type_code` (`dict_type`,`dict_code`) COMMENT '类型编码唯一索引',
  KEY `idx_dict_type` (`dict_type`) COMMENT '字典类型索引',
  KEY `idx_sort_order` (`sort_order`) COMMENT '排序索引',
  KEY `idx_status` (`status`) COMMENT '状态索引'
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='系统数据字典表，统一管理所有状态、类型的枚举值，便于维护和多语言扩展。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `data_dictionaries`
--

LOCK TABLES `data_dictionaries` WRITE;
/*!40000 ALTER TABLE `data_dictionaries` DISABLE KEYS */;
INSERT INTO `data_dictionaries` VALUES (1,'user_status','1','正常',1,1,1,'用户正常状态','2025-12-11 15:05:59','2025-12-11 15:05:59'),(2,'user_status','0','冻结',2,0,1,'用户被冻结','2025-12-11 15:05:59','2025-12-11 15:05:59'),(3,'user_status','2','注销',3,0,1,'用户已注销','2025-12-11 15:05:59','2025-12-11 15:05:59'),(4,'order_status','0','待付款',1,1,1,'等待用户支付','2025-12-11 15:05:59','2025-12-11 15:05:59'),(5,'order_status','1','待使用',2,0,1,'已支付，等待使用','2025-12-11 15:05:59','2025-12-11 15:05:59'),(6,'order_status','2','已完成',3,0,1,'订单已完成','2025-12-11 15:05:59','2025-12-11 15:05:59'),(7,'order_status','3','已取消',4,0,1,'订单已取消','2025-12-11 15:05:59','2025-12-11 15:05:59'),(8,'order_status','4','退款中',5,0,1,'正在退款处理中','2025-12-11 15:05:59','2025-12-11 15:05:59'),(9,'payment_status','0','待支付',1,1,1,'等待支付','2025-12-11 15:05:59','2025-12-11 15:05:59'),(10,'payment_status','1','成功',2,0,1,'支付成功','2025-12-11 15:05:59','2025-12-11 15:05:59'),(11,'payment_status','2','失败',3,0,1,'支付失败','2025-12-11 15:05:59','2025-12-11 15:05:59'),(12,'payment_status','3','已退款',4,0,1,'已退款','2025-12-11 15:05:59','2025-12-11 15:05:59'),(13,'payment_status','4','处理中',5,0,1,'支付处理中','2025-12-11 15:05:59','2025-12-11 15:05:59'),(14,'product_type','1','门票',1,1,1,'景区门票','2025-12-11 15:05:59','2025-12-11 15:05:59'),(15,'product_type','2','餐饮',2,0,1,'餐饮商品','2025-12-11 15:05:59','2025-12-11 15:05:59'),(16,'product_type','3','文创',3,0,1,'文创商品','2025-12-11 15:05:59','2025-12-11 15:05:59'),(17,'product_type','4','生鲜',4,0,1,'生鲜商品','2025-12-11 15:05:59','2025-12-11 15:05:59'),(18,'merchant_type','1','景区',1,1,1,'景区商户','2025-12-11 15:05:59','2025-12-11 15:05:59'),(19,'merchant_type','2','餐饮',2,0,1,'餐饮商户','2025-12-11 15:05:59','2025-12-11 15:05:59'),(20,'merchant_type','3','文创',3,0,1,'文创商户','2025-12-11 15:05:59','2025-12-11 15:05:59'),(21,'merchant_type','4','生鲜便利',4,0,1,'生鲜便利商户','2025-12-11 15:05:59','2025-12-11 15:05:59'),(22,'audit_status','0','待审核',1,1,1,'等待审核','2025-12-11 15:05:59','2025-12-11 15:05:59'),(23,'audit_status','1','通过',2,0,1,'审核通过','2025-12-11 15:05:59','2025-12-11 15:05:59'),(24,'audit_status','2','拒绝',3,0,1,'审核拒绝','2025-12-11 15:05:59','2025-12-11 15:05:59');
/*!40000 ALTER TABLE `data_dictionaries` ENABLE KEYS */;
UNLOCK TABLES;
...
INSERT INTO `flyway_schema_history` VALUES (1,'1','<< Flyway Baseline >>','BASELINE','<< Flyway Baseline >>',NULL,'root','2025-12-12 03:53:12',0,1),(2,'2','add timestamps to users','SQL','V2__add_timestamps_to_users.sql',1932653774,'root','2025-12-12 04:09:07',122,1);
...
CREATE TABLE `invoices` (
  ...
  `order_no` varchar(32) DEFAULT NULL COMMENT '关联订单(可为空，支持合并开票)',
  ...
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_invoice_no` (`invoice_no`) COMMENT '发票号码索引',
  KEY `idx_status` (`status`) COMMENT '开票状态索引',
  KEY `idx_applied_at` (`applied_at`) COMMENT '申请时间索引',
  CONSTRAINT `fk_invoices_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户发票申请与记录表。可与订单1对1或1对多。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `merchant_employee_role_assignments`
--

DROP TABLE IF EXISTS `merchant_employee_role_assignments`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商户员工与角色分配关系表。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `merchant_employee_role_assignments`
--

LOCK TABLES `merchant_employee_role_assignments` WRITE;
/*!40000 ALTER TABLE `merchant_employee_role_assignments` DISABLE KEYS */;
/*!40000 ALTER TABLE `merchant_employee_role_assignments` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `merchant_employee_roles`
--

DROP TABLE IF EXISTS `merchant_employee_roles`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `merchant_employee_roles` (
  ...
  `permissions` json DEFAULT NULL COMMENT '该角色拥有的权限集合，JSON格式',
  `is_system` tinyint(1) DEFAULT '0' COMMENT '是否为系统预置角色',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商户员工角色定义表，用于细粒度权限管理。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `merchant_employee_roles`
--

LOCK TABLES `merchant_employee_roles` WRITE;
/*!40000 ALTER TABLE `merchant_employee_roles` DISABLE KEYS */;
/*!40000 ALTER TABLE `merchant_employee_roles` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `merchant_employees` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '员工ID，主键自增',
  `merchant_id` bigint NOT NULL COMMENT '商户ID',
  ...
  `create_time` datetime DEFAULT (now()) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_merchant_user` (`merchant_id`,`user_id`) COMMENT '商户用户唯一索引',
  KEY `idx_merchant_id` (`merchant_id`) COMMENT '商户ID索引',
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_role` (`role`) COMMENT '角色类型索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商户员工绑定表。细粒度权限通过角色表(merchant_employee_roles)管理。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `merchant_settlements`
--
...
CREATE TABLE `merchant_settlements` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '结算ID，主键自增',
  `settlement_no` varchar(32) NOT NULL COMMENT '结算单号，如 SETTLE202412001',
  `merchant_id` bigint NOT NULL COMMENT '商户ID',
  ...
  KEY `idx_merchant_id` (`merchant_id`) COMMENT '商户ID索引',
  KEY `idx_settlement_no` (`settlement_no`) COMMENT '结算单号索引',
  KEY `idx_period` (`period`) COMMENT '结算周期索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='按周期给商户的结算单主表。明细见 settlement_details (下张表)。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `merchant_settlements`
--

LOCK TABLES `merchant_settlements` WRITE;
/*!40000 ALTER TABLE `merchant_settlements` DISABLE KEYS */;
/*!40000 ALTER TABLE `merchant_settlements` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `merchant_wallet` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '钱包ID，主键自增',
  `merchant_id` bigint NOT NULL COMMENT '商户ID，唯一',
  `balance` decimal(12,2) DEFAULT '0.00' COMMENT '可提现余额',
  `frozen_balance` decimal(12,2) DEFAULT '0.00' COMMENT '冻结余额(结算中/提现中)',
  ...
  `withdraw_password_hash` varchar(128) DEFAULT NULL COMMENT '提现密码哈希',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商户钱包，用于存放可提现的结算收入。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `merchants` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '商户ID，主键自增',
  ...
  `longitude` decimal(10,6) DEFAULT NULL COMMENT '经度，用于地图和附近推荐',
  `latitude` decimal(10,6) DEFAULT NULL COMMENT '纬度，用于地图和附近推荐',
  `settlement_rate` decimal(5,4) DEFAULT NULL COMMENT '平台结算费率',
  `audit_status` tinyint DEFAULT '0' COMMENT '审核状态: 0-待审核, 1-通过, 2-拒绝',
  `status` tinyint DEFAULT '1' COMMENT '状态: 1-正常, 0-停用',
  `level` tinyint DEFAULT '1' COMMENT '商户等级',
  `score` decimal(3,2) DEFAULT '5.00' COMMENT '商户评分，1-5分',
  `create_time` datetime DEFAULT (now()) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_city` (`city`) COMMENT '城市索引',
  KEY `idx_type` (`type`) COMMENT '商户类型索引',
  KEY `idx_cooperation_type` (`cooperation_type`) COMMENT '合作类型索引',
  KEY `idx_audit_status` (`audit_status`) COMMENT '审核状态索引',
  KEY `idx_status` (`status`) COMMENT '状态索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='入驻商户信息表，支持多种合作类型和结算规则。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `operation_logs` (
  ...
  `user_id` bigint DEFAULT NULL COMMENT '操作人ID，可为空(系统操作)',
  `user_type` tinyint DEFAULT NULL COMMENT '用户类型: 1-平台用户, 2-商户, 3-管理员',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_module` (`module`) COMMENT '操作模块索引',
  KEY `idx_created_at` (`created_at`) COMMENT '创建时间索引',
  KEY `idx_is_success` (`is_success`) COMMENT '成功状态索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='系统关键操作审计日志表，用于安全追踪和行为分析。建议详细日志保留6个月。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `order_changes` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '改签ID，主键自增',
  `change_no` varchar(32) NOT NULL COMMENT '改签单号',
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `change_type` tinyint NOT NULL COMMENT '类型: 1-改日期, 2-改时间, 3-改票种',
  `original_ticket_id` bigint DEFAULT NULL COMMENT '原票种ID',
  `new_ticket_id` bigint DEFAULT NULL COMMENT '新票种ID',
  `original_use_date` date DEFAULT NULL COMMENT '原使用日期',
  `new_use_date` date DEFAULT NULL COMMENT '新使用日期',
  `price_difference` decimal(10,2) DEFAULT NULL COMMENT '差价，正为需补，负为应退',
  `change_fee` decimal(10,2) DEFAULT '0.00' COMMENT '改签手续费',
  ...
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_change_no` (`change_no`) COMMENT '改签单号索引',
  KEY `idx_status` (`status`) COMMENT '状态索引',
  KEY `fk_order_changes_original_ticket` (`original_ticket_id`),
  KEY `fk_order_changes_new_ticket` (`new_ticket_id`),
  CONSTRAINT `fk_order_changes_new_ticket` FOREIGN KEY (`new_ticket_id`) REFERENCES `tickets` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_order_changes_order` FOREIGN KEY (`order_no`) REFERENCES `orders` (`order_no`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `fk_order_changes_original_ticket` FOREIGN KEY (`original_ticket_id`) REFERENCES `tickets` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_order_changes_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='门票订单改签记录表。涉及库存释放和重新锁定。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `order_changes`
--
...
CREATE TABLE `order_items` (
  ...
  `sku_specs` varchar(500) DEFAULT NULL COMMENT '下单时的规格快照',
  `unit_price` decimal(10,2) NOT NULL COMMENT '单价',
  `quantity` int NOT NULL COMMENT '购买数量',
  `subtotal` decimal(10,2) NOT NULL COMMENT '小计金额',
  `verification_code` varchar(50) DEFAULT NULL COMMENT '核销码/券码，唯一',
  `verification_status` tinyint DEFAULT '0' COMMENT '核销状态: 0-未核销, 1-已核销, 2-已过期',
  `ticket_date` date DEFAULT NULL COMMENT '票务使用日期',
  `visitor_name` varchar(50) DEFAULT NULL COMMENT '游客姓名（明文，仅用于核验时显示）',
  `visitor_name_encrypted` varchar(128) DEFAULT NULL COMMENT '加密存储的游客姓名',
  PRIMARY KEY (`id`),
  UNIQUE KEY `verification_code` (`verification_code`),
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
  KEY `idx_verification_code` (`verification_code`) COMMENT '核销码索引',
  KEY `idx_ticket_date` (`ticket_date`) COMMENT '使用日期索引',
  KEY `idx_verification_status` (`verification_status`) COMMENT '核销状态索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订单明细表，记录每件商品信息。核销码在此表管理。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `order_refunds` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '退款ID，主键自增',
  `refund_no` varchar(32) NOT NULL COMMENT '退款单号，如 REFUND202412110001',
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `refund_type` tinyint NOT NULL COMMENT '退款类型: 1-全额退款, 2-部分退款',
  `refund_amount` decimal(10,2) NOT NULL COMMENT '退款金额',
  `refund_reason` varchar(200) NOT NULL COMMENT '退款原因',
  `refund_evidence` json DEFAULT NULL COMMENT '退款凭证图片等，JSON格式',
  `status` tinyint DEFAULT '0' COMMENT '状态: 0-待审核, 1-审核通过, 2-审核拒绝, 3-退款中, 4-成功, 5-失败',
  `auditor_id` bigint DEFAULT NULL COMMENT '审核人ID',
  `audited_at` datetime DEFAULT NULL COMMENT '审核时间',
  `audit_note` varchar(500) DEFAULT NULL COMMENT '审核备注',
  `payment_refund_no` varchar(64) DEFAULT NULL COMMENT '支付渠道退款流水号',
  `payment_refund_at` datetime DEFAULT NULL COMMENT '支付渠道退款时间',
  `created_at` datetime DEFAULT (now()) COMMENT '创建时间',
  `updated_at` datetime DEFAULT (now()) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_refund_no` (`refund_no`) COMMENT '退款单号索引',
  KEY `idx_status` (`status`) COMMENT '状态索引',
  KEY `idx_created_at` (`created_at`) COMMENT '创建时间索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订单退款申请与处理表。与 order_changes 共同构成售后体系。建议终态记录保留3年。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `orders` (
  `order_no` varchar(32) NOT NULL COMMENT '业务订单号，如: ORDER202412110001',
  ...
  `payment_method` varchar(50) DEFAULT NULL COMMENT '支付方式: wechat, alipay, balance',
  ...
  `outer_order_no` varchar(64) DEFAULT NULL COMMENT '外部订单号(如OTA订单号)',
  `create_time` datetime DEFAULT (now()) COMMENT '创建时间',
  ...
  KEY `idx_user_status_time` (`user_id`,`order_status`,`create_time` DESC) COMMENT '用户状态时间联合索引',
  KEY `idx_merchant_status_time` (`merchant_id`,`order_status`,`create_time` DESC) COMMENT '商户状态时间联合索引',
  KEY `idx_outer_order_no` (`outer_order_no`) COMMENT '外部订单号索引',
  CONSTRAINT `fk_orders_merchant` FOREIGN KEY (`merchant_id`) REFERENCES `merchants` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_orders_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订单主表，按user_id分片，需考虑按create_time分表。状态机复杂，需仔细设计。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `payments` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '支付ID，主键自增',
  `payment_no` varchar(32) NOT NULL COMMENT '支付系统流水号',
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  ...
  `third_party_no` varchar(64) DEFAULT NULL COMMENT '第三方支付平台流水号',
  `payer_id` varchar(100) DEFAULT NULL COMMENT '支付方标识（如微信openid）',
  ...
  KEY `idx_third_party_no` (`third_party_no`) COMMENT '第三方流水号索引',
  KEY `idx_create_time` (`create_time`) COMMENT '创建时间索引',
  KEY `idx_status` (`status`) COMMENT '状态索引',
  CONSTRAINT `fk_payments_order` FOREIGN KEY (`order_no`) REFERENCES `orders` (`order_no`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='支付流水表，与订单1:1或1:N。需严格对账。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `product_skus` (
  ...
  `cost_price` decimal(10,2) DEFAULT NULL COMMENT '成本价，用于核算',
  ...
  KEY `idx_sku_code` (`sku_code`) COMMENT 'SKU编码索引',
  KEY `idx_status` (`status`) COMMENT '状态索引',
  KEY `idx_product_status` (`product_id`,`status`) COMMENT '商品状态联合索引',
  CONSTRAINT `fk_product_skus_product` FOREIGN KEY (`product_id`) REFERENCES `products` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商品最小库存单位表(SKU)，管理价格、库存和销售。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `products` (
  ...
  `scenic_spot_id` bigint DEFAULT NULL COMMENT '关联景区ID，非景区商品可为空',
  ...
  `image_list` json DEFAULT NULL COMMENT '商品图册，JSON数组格式',
  `description` text COMMENT '商品描述',
  `spec_template` json DEFAULT NULL COMMENT '规格模板，JSON格式',
  `point_ratio` decimal(5,4) DEFAULT '0.0100' COMMENT '积分赠送比例',
  `audit_status` tinyint DEFAULT '0' COMMENT '审核状态: 0-待审, 1-通过, 2-拒绝',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_merchant_id` (`merchant_id`) COMMENT '商户ID索引',
  KEY `idx_type` (`type`) COMMENT '商品类型索引',
  KEY `idx_scenic_spot_id` (`scenic_spot_id`) COMMENT '景区ID索引',
  KEY `idx_shelf_status` (`shelf_status`) COMMENT '上架状态索引',
  KEY `idx_audit_status` (`audit_status`) COMMENT '审核状态索引',
  CONSTRAINT `fk_products_merchant` FOREIGN KEY (`merchant_id`) REFERENCES `merchants` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_products_scenic_spot` FOREIGN KEY (`scenic_spot_id`) REFERENCES `scenic_spots` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='标准化商品信息表(SPU)，与具体规格(SKU)分离。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `scenic_daily_stats`
--

DROP TABLE IF EXISTS `scenic_daily_stats`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='景区维度日度统计汇总表，用于加速报表查询。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `scenic_daily_stats`
--

LOCK TABLES `scenic_daily_stats` WRITE;
/*!40000 ALTER TABLE `scenic_daily_stats` DISABLE KEYS */;
/*!40000 ALTER TABLE `scenic_daily_stats` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `scenic_spots` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '景区ID，主键自增',
  `name` varchar(200) NOT NULL COMMENT '景区名称',
  `level` varchar(20) DEFAULT NULL COMMENT '景区级别，如: A, AA, 5A',
  ...
  `opening_hours` varchar(200) DEFAULT NULL COMMENT '开放时间描述',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_city` (`city`) COMMENT '城市索引',
  KEY `idx_level` (`level`) COMMENT '景区级别索引',
  KEY `idx_status` (`status`) COMMENT '状态索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='文旅资源基础信息表，可扩展为多类型资源（博物馆、公园等）。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `seckill_events`
--
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='秒杀/闪购活动专用表，独立库存以应对高并发。需配合缓存和队列。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `seckill_events`
--

LOCK TABLES `seckill_events` WRITE;
/*!40000 ALTER TABLE `seckill_events` DISABLE KEYS */;
/*!40000 ALTER TABLE `seckill_events` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `settlement_details` (
  ...
  `settled_at` datetime DEFAULT NULL COMMENT '计入结算时间',
  `status` tinyint DEFAULT '1' COMMENT '状态: 1-正常, 0-异常(如退款需冲正)',
  PRIMARY KEY (`id`),
  KEY `idx_settlement_id` (`settlement_id`) COMMENT '结算单ID索引',
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='结算单明细，记录每一笔订单的分账构成。退款订单会产生冲正明细。';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `settlement_details`
--

LOCK TABLES `settlement_details` WRITE;
/*!40000 ALTER TABLE `settlement_details` DISABLE KEYS */;
/*!40000 ALTER TABLE `settlement_details` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `ticket_prices` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '票价ID，主键自增',
  ...
  `is_active` tinyint(1) DEFAULT '1' COMMENT '该日期价格是否生效',
  `create_time` datetime DEFAULT (now()) COMMENT '创建时间',
  `update_time` datetime DEFAULT (now()) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_ticket_date_type` (`ticket_id`,`price_date`,`price_type`) COMMENT '票种日期类型唯一索引',
  KEY `idx_ticket_id` (`ticket_id`) COMMENT '票种ID索引',
  KEY `idx_price_date` (`price_date`) COMMENT '适用日期索引',
  KEY `idx_date_active` (`price_date`,`is_active`) COMMENT '日期生效状态联合索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='票价与库存日历表。支持每天不同的价格和独立的库存管理，是实际销售的单位。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `tickets` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '票种ID，主键自增',
  `scenic_spot_id` bigint NOT NULL COMMENT '景区ID',
  ...
  `valid_days` int DEFAULT NULL COMMENT '有效期天数（当validity_type=2时使用）',
  ...
  `limit_per_day` int DEFAULT NULL COMMENT '每日限购数量',
  `require_real_name` tinyint(1) DEFAULT '1' COMMENT '是否需要实名',
  `require_id_card` tinyint(1) DEFAULT '1' COMMENT '是否需要身份证',
  `verification_mode` tinyint DEFAULT '1' COMMENT '核验方式: 1-二维码, 2-人脸, 3-身份证',
  `status` tinyint DEFAULT '1' COMMENT '状态: 1-上架, 0-下架',
  `create_time` datetime DEFAULT (now()) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_scenic_spot_id` (`scenic_spot_id`) COMMENT '景区ID索引',
  KEY `idx_type` (`type`) COMMENT '票种类型索引',
  KEY `idx_status` (`status`) COMMENT '状态索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='票种模板表，定义某景区的一种门票规则。实际销售的是其下某一天的具体库存（ticket_prices）。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `transaction_messages` (
  ...
  `created_at` datetime DEFAULT (now()) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_status_retry` (`status`,`next_retry_time`) COMMENT '状态重试时间索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='本地消息表，用于实现分布式事务最终一致性。需定期清理已完成/失败消息。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_addresses` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_user_default` (`user_id`,`is_default`) COMMENT '用户默认地址索引',
  CONSTRAINT `fk_user_addresses_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户收货地址表，用于实物商品（文创、生鲜）邮寄。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_comments` (
  ...
  `images` json DEFAULT NULL COMMENT '评价图片，JSON格式',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_target_type` (`target_type`) COMMENT '评价对象类型索引',
  KEY `idx_target_id` (`target_id`) COMMENT '评价对象ID索引',
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户评价表。支持对商户、商品、活动的多维度评价，是信用体系基础。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_coupons` (
  ...
  `coupon_code` varchar(50) NOT NULL COMMENT '唯一券码',
  ...
  KEY `idx_status` (`status`) COMMENT '状态索引',
  KEY `idx_expires_at` (`expires_at`) COMMENT '过期时间索引',
  KEY `idx_coupon_code` (`coupon_code`) COMMENT '券码索引',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户实际领取和持有的优惠券。由 coupons 模板生成。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_feedback` (
  ...
  `images` json DEFAULT NULL COMMENT '上传的凭证图片，JSON格式',
  `contact_phone` varchar(20) DEFAULT NULL COMMENT '联系电话',
  `order_no` varchar(32) DEFAULT NULL COMMENT '关联订单',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`) COMMENT '用户ID索引',
  KEY `idx_type` (`type`) COMMENT '反馈类型索引',
  KEY `idx_status` (`status`) COMMENT '处理状态索引',
  KEY `idx_created_at` (`created_at`) COMMENT '创建时间索引',
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
  KEY `fk_user_feedback_merchant` (`merchant_id`),
  CONSTRAINT `fk_user_feedback_merchant` FOREIGN KEY (`merchant_id`) REFERENCES `merchants` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_user_feedback_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户反馈与工单表。用于客服跟踪和处理。建议已处理记录保留1年。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_points` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '流水ID，主键自增',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  ...
  KEY `idx_expires_at` (`expires_at`) COMMENT '过期时间索引',
  KEY `idx_order_no` (`order_no`) COMMENT '订单号索引',
  KEY `idx_expires_status` (`expires_at`,`status`) COMMENT '过期状态联合索引',
  CONSTRAINT `fk_user_points_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户积分流水表，所有积分变动有迹可循。需定期清理过期积分记录（建议无效记录保留2年）。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_wallet` (
  ...
  `frozen_balance` decimal(10,2) DEFAULT '0.00' COMMENT '冻结金额(如提现中、支付中)',
  `total_deposit` decimal(10,2) DEFAULT '0.00' COMMENT '累计充值',
  `total_consumption` decimal(10,2) DEFAULT '0.00' COMMENT '累计消费',
  `security_level` tinyint DEFAULT '1' COMMENT '安全等级',
  `pay_password_hash` varchar(128) DEFAULT NULL COMMENT '支付密码哈希',
  `status` tinyint DEFAULT '1' COMMENT '状态: 1-正常, 0-冻结',
  `created_at` datetime DEFAULT (now()) COMMENT '创建时间',
  `updated_at` datetime DEFAULT (now()) COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户钱包（余额）账户，独立于积分体系，用于储值消费。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `users` (
  ...
  `id_card_encrypted` varchar(128) DEFAULT NULL COMMENT '身份证号加密存储',
  ...
  `wallet_balance` decimal(10,2) DEFAULT '0.00' COMMENT '钱包余额(元)',
  ...
  KEY `idx_phone_encrypted` (`phone_encrypted`) COMMENT '加密手机号索引',
  KEY `idx_member_level` (`member_level`) COMMENT '会员等级索引',
  KEY `idx_register_time` (`register_time`) COMMENT '注册时间索引',
  KEY `idx_status` (`status`) COMMENT '状态索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='平台用户核心信息表，预计数据量50万+。敏感信息均加密存储。建议数据保留至注销后3年。';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2025-12-12 12:52:44

...
CREATE TABLE `user` (
  ...
  `phone_encrypted` varchar(128) COMMENT '手机号加密值',
  `nickname` varchar(50) NOT NULL COMMENT '昵称',
  `avatar_url` varchar(500) COMMENT '头像URL',
  `id_card` varchar(128) COMMENT '身份证加密值',
  `real_name` varchar(50) COMMENT '真实姓名',
  `gender` tinyint COMMENT '性别:0未知1男2女',
  `birthday` date COMMENT '生日',
  `member_level` int NOT NULL DEFAULT 1 COMMENT '会员等级:1普通2白银3黄金4钻石',
  ...
  `status` tinyint NOT NULL DEFAULT 0 COMMENT '状态:0正常1冻结2注销',
  `data_version` int NOT NULL DEFAULT 1 COMMENT '数据版本(乐观锁)',
  PRIMARY KEY (`id`),
...
CREATE TABLE `user_favorite` (
  ...
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `favorite_type` tinyint NOT NULL COMMENT '收藏类型:1景区2商品3活动',
...
CREATE TABLE `user_coupon` (
  ...
  `coupon_type` tinyint NOT NULL COMMENT '优惠券类型:1满减2折扣3立减',
  `coupon_value` decimal(10,2) NOT NULL COMMENT '优惠值',
  `min_amount` decimal(10,2) COMMENT '最低使用金额',
  `applicable_type` tinyint NOT NULL COMMENT '适用范围:1全平台2指定商户3指定商品',
  `applicable_ids` json COMMENT '适用ID列表',
  `validity_type` tinyint NOT NULL COMMENT '有效期类型:1固定时间2领取后生效',
  `start_time` datetime COMMENT '开始时间',
  `end_time` datetime COMMENT '结束时间',
  `validity_days` int COMMENT '有效天数',
  `receive_time` datetime NOT NULL COMMENT '领取时间',
  `expire_time` datetime NOT NULL COMMENT '过期时间',
  `use_status` tinyint NOT NULL DEFAULT 0 COMMENT '使用状态:0未使用1已使用2已过期',
...
CREATE TABLE `user_point` (
  ...
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `point_type` tinyint NOT NULL COMMENT '积分类型:1获取2消耗',
  `point_source` varchar(50) NOT NULL COMMENT '积分来源:购票/消费/签到/活动',
  `point_amount` int NOT NULL COMMENT '积分数量',
  `available_points` int NOT NULL COMMENT '可用积分余额',
  `total_points` int NOT NULL COMMENT '总积分余额',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`),
  KEY `idx_point_source` (`point_source`),
  KEY `idx_create_time` (`create_time`),
  KEY `idx_expire_time` (`expire_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户积分表';
```

**表：user_feedback（用户反馈表）**

```sql
CREATE TABLE `user_feedback` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '反馈ID',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `feedback_type` tinyint NOT NULL COMMENT '反馈类型:1投诉2建议3咨询',
  ...
  `status` tinyint NOT NULL DEFAULT 0 COMMENT '状态:0待处理1处理中2已处理',
...
**表：user_evaluation（用户评价表）**

```sql
CREATE TABLE `user_evaluation` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '评价ID',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `evaluation_type` tinyint NOT NULL COMMENT '评价类型:1景区2商品3活动',
...
#### 4.2.2 订单相关表（表 T-4.2.2）

**表：order（订单表）**

```sql
CREATE TABLE `order` (
  `order_no` varchar(32) NOT NULL COMMENT '订单号(业务生成)',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `merchant_id` bigint COMMENT '商户ID(可为空)',
  `order_type` tinyint NOT NULL COMMENT '订单类型:1门票2商品3活动',
  `order_source` varchar(50) NOT NULL COMMENT '订单来源:小程序/PC/API',
  ...
  `point_earned` int NOT NULL DEFAULT 0 COMMENT '获得积分',
  `payment_method` varchar(50) COMMENT '支付方式:微信/支付宝',
  `payment_status` tinyint NOT NULL DEFAULT 0 COMMENT '支付状态:0待支付1支付成功2支付失败3已退款',
  `order_status` tinyint NOT NULL DEFAULT 0 COMMENT '订单状态:0待付款1待使用2已完成3已取消4退款中5已退款',
  `refund_status` tinyint NOT NULL DEFAULT 0 COMMENT '退款状态:0无退款1退款中2退款成功3退款失败',
...
-- 说明：此表按user_id分片，按create_time按月分表
```
...
CREATE TABLE `order_item` (
  ...
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `item_type` tinyint NOT NULL COMMENT '明细类型:1门票2商品3活动',
  `product_id` bigint COMMENT '商品ID',
  `product_name` varchar(200) NOT NULL COMMENT '商品名称',
  `sku_id` bigint COMMENT 'SKU ID',
  `sku_spec` varchar(500) COMMENT '规格信息',
  `unit_price` decimal(10,2) NOT NULL COMMENT '单价',
  `quantity` int NOT NULL COMMENT '数量',
  `subtotal` decimal(10,2) NOT NULL COMMENT '小计金额',
  `ticket_date` date COMMENT '票务使用日期',
  `ticket_time` time COMMENT '票务使用时间',
  `visitor_name` varchar(50) COMMENT '游客姓名',
  `visitor_id_card` varchar(128) COMMENT '游客身份证加密',
  `verification_code` varchar(50) COMMENT '核销码',
  `verification_status` tinyint NOT NULL DEFAULT 0 COMMENT '核销状态:0未核销1已核销2已过期',
  `verification_time` datetime COMMENT '核销时间',
  `verification_operator` varchar(50) COMMENT '核销操作员',
...
CREATE TABLE `order_refund` (
  ...
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `refund_type` tinyint NOT NULL COMMENT '退款类型:1全额退款2部分退款',
  `refund_amount` decimal(10,2) NOT NULL COMMENT '退款金额',
  `refund_reason` varchar(200) NOT NULL COMMENT '退款原因',
  `refund_images` json COMMENT '退款凭证图片',
  `refund_status` tinyint NOT NULL DEFAULT 0 COMMENT '退款状态:0待审核1审核通过2审核拒绝3退款中4退款成功5退款失败',
  `auditor_id` bigint COMMENT '审核人ID',
  `audit_time` datetime COMMENT '审核时间',
  `audit_remark` varchar(500) COMMENT '审核备注',
  `payment_refund_no` varchar(64) COMMENT '支付渠道退款单号',
  `payment_refund_time` datetime COMMENT '支付渠道退款时间',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
**表：order_change（订单改签表）**

```sql
CREATE TABLE `order_change` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '改签ID',
  `change_no` varchar(32) NOT NULL COMMENT '改签单号',
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `change_type` tinyint NOT NULL COMMENT '改签类型:1改日期2改时间3改票种',
  `original_date` date COMMENT '原使用日期',
  `new_date` date COMMENT '新使用日期',
  `original_ticket_id` bigint COMMENT '原票种ID',
  `new_ticket_id` bigint COMMENT '新票种ID',
  `price_difference` decimal(10,2) COMMENT '差价',
  `change_fee` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '改签手续费',
  `total_amount` decimal(10,2) NOT NULL COMMENT '改签总金额',
  `change_reason` varchar(200) NOT NULL COMMENT '改签原因',
  `change_status` tinyint NOT NULL DEFAULT 0 COMMENT '改签状态:0待审核1审核通过2审核拒绝3已完成',
  `auditor_id` bigint COMMENT '审核人ID',
  `audit_time` datetime COMMENT '审核时间',
  `audit_remark` varchar(500) COMMENT '审核备注',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  ...
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='订单改签表';
```

**表：order_visitor（订单游客表）**

```sql
CREATE TABLE `order_visitor` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '游客ID',
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `visitor_name` varchar(50) NOT NULL COMMENT '游客姓名',
  `visitor_phone` varchar(20) COMMENT '游客电话',
  `visitor_id_card` varchar(128) NOT NULL COMMENT '游客身份证加密',
  `visitor_type` tinyint NOT NULL COMMENT '游客类型:1成人2儿童3学生4老人',
  `age` int COMMENT '年龄',
  `gender` tinyint COMMENT '性别:0未知1男2女',
  `is_contact` tinyint NOT NULL DEFAULT 0 COMMENT '是否为联系人:0否1是',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_order_no` (`order_no`),
  KEY `idx_visitor_id_card` (`visitor_id_card`(20))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='订单游客表';
```

#### 4.2.3 商品相关表（表 T-4.2.3）
...
CREATE TABLE `product` (
  ...
  `product_type` tinyint NOT NULL COMMENT '商品类型:1门票2餐饮3文创4生鲜',
  `category_id` bigint NOT NULL COMMENT '分类ID',
  `product_name` varchar(200) NOT NULL COMMENT '商品名称',
  `product_subtitle` varchar(200) COMMENT '商品副标题',
  `main_image` varchar(500) COMMENT '主图URL',
  `image_list` json COMMENT '图片列表',
  `video_url` varchar(500) COMMENT '视频URL',
  `description` text COMMENT '商品描述',
  `spec_template` json COMMENT '规格模板',
  `point_ratio` decimal(5,4) NOT NULL DEFAULT 0.0100 COMMENT '积分比例',
  `settlement_ratio` decimal(5,4) NOT NULL COMMENT '结算比例',
  `audit_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
  `shelf_status` tinyint NOT NULL DEFAULT 0 COMMENT '上架状态:0下架1上架2定时上架',
  ...
  FULLTEXT KEY `ft_product_name` (`product_name`, `product_subtitle`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品表';
```

**表：product_sku（商品 SKU 表）**
...
CREATE TABLE `product_category` (
  ...
  `category_type` tinyint NOT NULL COMMENT '分类类型:1门票2餐饮3文创4生鲜',
...
**表：product_stock_log（商品库存流水表）**

```sql
CREATE TABLE `product_stock_log` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '流水ID',
  `sku_id` bigint NOT NULL COMMENT 'SKU ID',
  `change_type` tinyint NOT NULL COMMENT '变动类型:1入库2出库3锁定4释放5调整',
  ...
  `operator_type` tinyint NOT NULL COMMENT '操作人类型:1系统2用户3商户4管理员',
  `remark` varchar(200) COMMENT '备注',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_order_no` (`order_no`),
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品库存流水表';
```

#### 4.2.4 票务相关表（表 T-4.2.4）
...
CREATE TABLE `ticket` (
  ...
  `ticket_type` tinyint NOT NULL COMMENT '票种类型:1单票2联票3套票',
  `ticket_name` varchar(200) NOT NULL COMMENT '门票名称',
  `description` text COMMENT '门票描述',
  `validity_type` tinyint NOT NULL COMMENT '有效期类型:1指定日期2有效期天数',
  `valid_days` int COMMENT '有效天数',
  `advance_days` int NOT NULL DEFAULT 7 COMMENT '可提前预订天数',
  `refund_rule` json NOT NULL COMMENT '退款规则JSON',
  `change_rule` json NOT NULL COMMENT '改签规则JSON',
  `limit_per_user` int NOT NULL DEFAULT 5 COMMENT '每用户限购数',
  `limit_per_day` int COMMENT '每日限购数',
  `require_real_name` tinyint NOT NULL DEFAULT 1 COMMENT '是否需要实名:0否1是',
  `require_id_card` tinyint NOT NULL DEFAULT 1 COMMENT '是否需要身份证:0否1是',
  `verification_mode` tinyint NOT NULL DEFAULT 1 COMMENT '核验方式:1二维码2人脸3身份证',
...
CREATE TABLE `ticket_price` (
  ...
  `weekday_price` decimal(10,2) COMMENT '工作日价格',
  ...
  `is_active` tinyint NOT NULL DEFAULT 1 COMMENT '是否生效:0否1是',
  PRIMARY KEY (`id`),
  KEY `idx_ticket_id` (`ticket_id`),
  KEY `idx_date_range` (`start_date`, `end_date`),
  KEY `idx_is_active` (`is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='门票价格表';
```
...
CREATE TABLE `scenic` (
  ...
  `scenic_level` varchar(50) COMMENT '景区级别',
  ...
  `stop_entry_time` time COMMENT '停止入园时间',
  `close_time` time COMMENT '闭园时间',
  `contact_phone` varchar(20) COMMENT '联系电话',
  `main_image` varchar(500) COMMENT '主图',
  `image_list` json COMMENT '图片列表',
  `video_url` varchar(500) COMMENT '视频URL',
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态:0关闭1开放',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_city` (`city`),
  KEY `idx_scenic_level` (`scenic_level`),
  FULLTEXT KEY `ft_scenic_name` (`scenic_name`, `introduction`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='景区表';
```

**表：ticket_verification（门票核销记录表）**
...
CREATE TABLE `ticket_verification` (
  ...
  `verification_type` tinyint NOT NULL COMMENT '核销类型:1扫码核销2人脸核销3身份证核销',
  `verification_time` datetime NOT NULL COMMENT '核销时间',
  `verification_device` varchar(100) COMMENT '核销设备',
  `verification_operator` varchar(50) COMMENT '核销操作员',
  `verification_location` varchar(200) COMMENT '核销地点',
  `longitude` decimal(10,6) COMMENT '经度',
  `latitude` decimal(10,6) COMMENT '纬度',
  ...
  KEY `idx_verification_operator` (`verification_operator`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='门票核销记录表';
```

#### 4.2.5 活动相关表（表 T-4.2.5）
...
CREATE TABLE `activity` (
  ...
  `merchant_id` bigint COMMENT '商户ID(平台活动为空)',
  `activity_type` tinyint NOT NULL COMMENT '活动类型:1打卡活动2积分活动3主题活动',
  ...
  `participation_count` int NOT NULL DEFAULT 0 COMMENT '已参与人数',
  `requirement_type` tinyint NOT NULL DEFAULT 0 COMMENT '参与条件:0无1需购票2需积分3需报名',
  `requirement_value` varchar(200) COMMENT '条件值',
  `reward_type` tinyint NOT NULL COMMENT '奖励类型:1积分2优惠券3实物4虚拟',
  `reward_config` json NOT NULL COMMENT '奖励配置JSON',
  `audit_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
  `activity_status` tinyint NOT NULL DEFAULT 0 COMMENT '活动状态:0未开始1进行中2已结束3已取消',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_merchant_id` (`merchant_id`),
  ...
  FULLTEXT KEY `ft_activity_name` (`activity_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='活动表';
```

**表：activity_participant（活动参与记录表）**
...
CREATE TABLE `activity_participant` (
  ...
  `participation_status` tinyint NOT NULL DEFAULT 1 COMMENT '参与状态:1进行中2已完成3已放弃',
  `completion_time` datetime COMMENT '完成时间',
  `reward_status` tinyint NOT NULL DEFAULT 0 COMMENT '奖励状态:0未发放1已发放2发放失败',
  `reward_detail` json COMMENT '奖励详情',
  `checkpoint_progress` json COMMENT '打卡点进度',
  `total_checkpoints` int NOT NULL DEFAULT 0 COMMENT '总打卡点',
  `completed_checkpoints` int NOT NULL DEFAULT 0 COMMENT '已完成打卡点',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_activity_user` (`activity_id`, `user_id`),
  KEY `idx_user_id` (`user_id`),
  KEY `idx_participation_time` (`participation_time`),
  KEY `idx_participation_status` (`participation_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='活动参与记录表';
```

**表：activity_checkpoint（活动打卡点表）**

```sql
CREATE TABLE `activity_checkpoint` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '打卡点ID',
  `activity_id` bigint NOT NULL COMMENT '活动ID',
  `checkpoint_name` varchar(100) NOT NULL COMMENT '打卡点名称',
  `checkpoint_description` text COMMENT '打卡点描述',
  `location` varchar(500) COMMENT '打卡点位置',
  `longitude` decimal(10,6) COMMENT '经度',
  `latitude` decimal(10,6) COMMENT '纬度',
  `checkpoint_type` tinyint NOT NULL COMMENT '打卡类型:1位置打卡2拍照打卡3答题打卡',
  `checkpoint_config` json COMMENT '打卡配置',
  ...
  KEY `idx_sort_order` (`sort_order`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='活动打卡点表';
```

**表：activity_checkpoint_record（打卡点记录表）**

```sql
CREATE TABLE `activity_checkpoint_record` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '记录ID',
  `checkpoint_id` bigint NOT NULL COMMENT '打卡点ID',
  `participant_id` bigint NOT NULL COMMENT '参与记录ID',
  `check_time` datetime NOT NULL COMMENT '打卡时间',
  `check_type` tinyint NOT NULL COMMENT '打卡方式:1自动2手动',
  `check_content` json COMMENT '打卡内容',
  `check_images` json COMMENT '打卡图片',
  `check_status` tinyint NOT NULL DEFAULT 1 COMMENT '打卡状态:1成功0失败',
  `review_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
  `reviewer_id` bigint COMMENT '审核人ID',
  `review_time` datetime COMMENT '审核时间',
  `review_remark` varchar(200) COMMENT '审核备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_checkpoint_participant` (`checkpoint_id`, `participant_id`),
  KEY `idx_participant_id` (`participant_id`),
  KEY `idx_check_time` (`check_time`),
  KEY `idx_check_status` (`check_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='打卡点记录表';
```

#### 4.2.6 财务相关表（表 T-4.2.6）
...
CREATE TABLE `payment_record` (
  ...
  `order_no` varchar(32) NOT NULL COMMENT '订单号',
  `payment_type` varchar(20) NOT NULL COMMENT '支付类型:微信/支付宝',
  `payment_channel` varchar(50) NOT NULL COMMENT '支付渠道',
  `payment_amount` decimal(10,2) NOT NULL COMMENT '支付金额',
  `payment_status` tinyint NOT NULL COMMENT '支付状态:0待支付1支付成功2支付失败3已退款',
  `third_party_no` varchar(64) COMMENT '第三方流水号',
  `payer_id` varchar(100) COMMENT '支付方标识',
  `payer_account` varchar(100) COMMENT '支付方账号',
...
CREATE TABLE `settlement` (
  ...
  `merchant_id` bigint NOT NULL COMMENT '商户ID',
  `settlement_period` varchar(20) NOT NULL COMMENT '结算周期:2025-12',
  `settlement_type` tinyint NOT NULL COMMENT '结算类型:1自动结算2手动结算',
  `total_amount` decimal(10,2) NOT NULL COMMENT '结算总金额',
  `platform_fee` decimal(10,2) NOT NULL COMMENT '平台服务费',
  `settlement_amount` decimal(10,2) NOT NULL COMMENT '应结金额',
  `settlement_status` tinyint NOT NULL DEFAULT 0 COMMENT '结算状态:0待结算1结算中2结算成功3结算失败',
  `payment_status` tinyint NOT NULL DEFAULT 0 COMMENT '付款状态:0待付款1付款中2付款成功3付款失败',
  `payment_method` varchar(50) COMMENT '付款方式',
  `payment_time` datetime COMMENT '付款时间',
  `payment_no` varchar(64) COMMENT '付款流水号',
  `invoice_status` tinyint NOT NULL DEFAULT 0 COMMENT '发票状态:0未开票1已开票',
  `audit_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
...
CREATE TABLE `settlement_detail` (
  ...
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态:1正常0异常',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `invoice` (
  ...
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `invoice_type` tinyint NOT NULL COMMENT '发票类型:1普通发票2专用发票',
  ...
  `invoice_status` tinyint NOT NULL DEFAULT 0 COMMENT '发票状态:0待开票1开票中2已开票3开票失败',
...
CREATE TABLE `wallet` (
  ...
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态:1正常0冻结',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `wallet_transaction` (
  ...
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `transaction_type` tinyint NOT NULL COMMENT '交易类型:1充值2消费3退款4提现',
  ...
  `order_no` varchar(32) COMMENT '关联订单号',
  `payment_no` varchar(32) COMMENT '关联支付单号',
  `remark` varchar(200) COMMENT '备注',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `merchant` (
  ...
  `merchant_type` tinyint NOT NULL COMMENT '商户类型:1景区2餐饮3文创4生鲜',
  `cooperation_type` tinyint NOT NULL COMMENT '合作类型:1直营2联营3加盟',
  ...
  `legal_person_id` varchar(128) COMMENT '法人身份证号加密',
  `settlement_account` varchar(100) COMMENT '结算账户',
  `settlement_bank` varchar(100) COMMENT '结算银行',
  `settlement_rate` decimal(5,4) NOT NULL COMMENT '结算费率',
  `audit_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
  `merchant_status` tinyint NOT NULL DEFAULT 1 COMMENT '商户状态:1正常0停用2黑名单',
  `level` tinyint NOT NULL DEFAULT 1 COMMENT '商户等级',
  `score` decimal(3,2) NOT NULL DEFAULT 5.00 COMMENT '商户评分',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `merchant_employee` (
  ...
  `employee_role` tinyint NOT NULL COMMENT '员工角色:1管理员2操作员3核销员',
...
CREATE TABLE `merchant_wallet` (
  ...
  `security_level` tinyint NOT NULL DEFAULT 1 COMMENT '安全等级',
  `withdraw_password` varchar(128) COMMENT '提现密码',
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态:1正常0冻结',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_balance` (`balance`),
  KEY `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商户钱包表';
```

**表：merchant_withdraw（商户提现表）**
...
CREATE TABLE `merchant_withdraw` (
  ...
  `withdraw_status` tinyint NOT NULL DEFAULT 0 COMMENT '提现状态:0待审核1审核通过2审核拒绝3打款中4打款成功5打款失败',
  `auditor_id` bigint COMMENT '审核人ID',
  `audit_time` datetime COMMENT '审核时间',
  `audit_remark` varchar(200) COMMENT '审核备注',
  `payment_time` datetime COMMENT '打款时间',
  `payment_no` varchar(64) COMMENT '打款流水号',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `system_config` (
  ...
  `config_type` tinyint NOT NULL DEFAULT 1 COMMENT '配置类型:1系统2业务3商户',
...
CREATE TABLE `system_log` (
  ...
  `log_type` tinyint NOT NULL COMMENT '日志类型:1操作日志2登录日志3错误日志',
  `user_id` bigint COMMENT '用户ID',
  `user_type` tinyint COMMENT '用户类型:1平台用户2商户用户3管理员',
  ...
  KEY `idx_user_id` (`user_id`),
  KEY `idx_operation` (`operation`(100)),
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='系统日志表';
```

**表：system_notice（系统通知表）**

```sql
CREATE TABLE `system_notice` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '通知ID',
  `notice_type` tinyint NOT NULL COMMENT '通知类型:1系统公告2活动通知3订单通知',
  `title` varchar(200) NOT NULL COMMENT '通知标题',
  `content` text NOT NULL COMMENT '通知内容',
  `target_type` tinyint NOT NULL COMMENT '目标类型:1所有用户2指定用户3指定商户',
  `target_ids` json COMMENT '目标ID列表',
  `publish_time` datetime NOT NULL COMMENT '发布时间',
  `expire_time` datetime COMMENT '过期时间',
  `status` tinyint NOT NULL DEFAULT 0 COMMENT '状态:0草稿1已发布2已过期',
...
CREATE TABLE `system_file` (
  ...
  `uploader_type` tinyint NOT NULL COMMENT '上传人类型:1用户2商户3管理员',
  `usage_type` tinyint NOT NULL COMMENT '用途类型:1商品图片2活动海报3用户头像4资质文件',
...
#### 4.2.9 供需相关表（表 T-4.2.9）

**表：supply_demand（供需表）**

```sql
CREATE TABLE `supply_demand` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '供需ID',
  `type` tinyint NOT NULL COMMENT '类型:1供应2需求',
  ...
  `publisher_type` tinyint NOT NULL COMMENT '发布人类型:1用户2商户',
  `audit_status` tinyint NOT NULL DEFAULT 0 COMMENT '审核状态:0待审核1审核通过2审核拒绝',
  `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态:1正常0关闭',
  `view_count` int NOT NULL DEFAULT 0 COMMENT '浏览数',
  `contact_count` int NOT NULL DEFAULT 0 COMMENT '联系数',
  `expire_time` datetime COMMENT '过期时间',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
**表：supply_demand_contact（供需联系记录表）**

```sql
CREATE TABLE `supply_demand_contact` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '联系记录ID',
  `supply_demand_id` bigint NOT NULL COMMENT '供需ID',
  `contact_user_id` bigint NOT NULL COMMENT '联系用户ID',
  `contact_type` tinyint NOT NULL COMMENT '联系类型:1电话2微信3邮箱',
  `contact_content` text COMMENT '联系内容',
  `contact_status` tinyint NOT NULL DEFAULT 0 COMMENT '联系状态:0待回复1已回复2已关闭',
  `create_time` datetime NOT NULL COMMENT '联系时间',
  `update_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_supply_demand_id` (`supply_demand_id`),
  KEY `idx_contact_user_id` (`contact_user_id`),
  KEY `idx_contact_status` (`contact_status`),
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='供需联系记录表';
```

#### 4.2.10 设备相关表（表 T-4.2.10）
...
CREATE TABLE `device` (
  ...
  `device_type` tinyint NOT NULL COMMENT '设备类型:1闸机2手持机3POS机4自助机',
  ...
  `device_status` tinyint NOT NULL DEFAULT 0 COMMENT '设备状态:0离线1在线2故障',
  `last_online_time` datetime COMMENT '最后在线时间',
  `version` varchar(50) COMMENT '设备版本',
...
CREATE TABLE `device_log` (
  ...
  `log_type` tinyint NOT NULL COMMENT '日志类型:1状态2操作3异常',
  `log_content` text NOT NULL COMMENT '日志内容',
  `log_level` tinyint NOT NULL COMMENT '日志级别:1信息2警告3错误',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),

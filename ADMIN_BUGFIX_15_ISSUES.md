...
### 【问题1】支付管理-订单号支持模糊检索
**问题描述**：订单号检索不支持模糊查询  
**修复方案**：后端已支持LIKE查询，前端直接传递orderNo参数即可  
**涉及文件**：
...
### 【问题3】支付管理-时间范围检索400错误
**问题描述**：选择时间范围后点击检索报400错误  
**错误信息**：`GET http://localhost:3000/api/payments?startTime=2025-12-16+00:00:00&endTime=2025-12-17+00:00:00 400`

**根本原因**：后端使用`@DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)`需要ISO格式（T分隔符），但前端传递的是空格分隔的格式
...
### 【问题5】支付管理-关闭按钮405错误
**问题描述**：点击"关闭"按钮提示405 Method Not Allowed  
**错误信息**：`PUT http://localhost:3000/api/payments/PAY202512160004/close 405`

**根本原因**：前端调用`closePayment`使用PUT方法，但后端定义为`@PostMapping`
...
**涉及文件**：
- `frontend/admin-web/src/views/wallet/WalletList.vue` (问题10)
- `frontend/admin-web/src/views/wallet/WalletTransactionList.vue` (问题11)
- `frontend/admin-web/src/views/points/PointsList.vue` (问题12)
- `frontend/admin-web/src/views/points/PointsTransactionList.vue` (问题13)
- `frontend/admin-web/src/views/coupons/CouponList.vue` (问题14)
- `frontend/admin-web/src/views/coupons/UserCouponList.vue` (问题15)
...
### 1. 日期格式处理
**问题**：Spring Boot默认使用ISO格式（`2025-12-16T00:00:00`），前端传递空格格式（`2025-12-16 00:00:00`）  

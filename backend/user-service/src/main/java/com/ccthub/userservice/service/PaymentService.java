...
public class PaymentService {
    ...
    public void updateOrderStatusAsync(String orderNo) {
        try {
            ...
            if (!Order.OrderStatus.PENDING_PAYMENT.equals(order.getOrderStatus())) {
                log.warn("订单状态不是待付款，无法更新，orderNo={}, currentStatus={}", orderNo, order.getOrderStatus());
                return;
            }
            ...
            log.info("订单状态更新成功，orderNo={}, orderStatus={}, paymentStatus={}",
                    orderNo, Order.OrderStatus.PENDING_USE, Order.PaymentStatus.SUCCESS);
        } catch (Exception e) {
    ...
    public PaymentResponse queryPaymentByOrderNo(String orderNo) {
        Payment payment = paymentRepository.findByOrderNoAndStatus(orderNo, Payment.STATUS_SUCCESS)
                .or(() -> paymentRepository.findByOrderNoAndStatus(orderNo, Payment.STATUS_PENDING))
                .orElseThrow(() -> new IllegalArgumentException("支付订单不存在"));
        return convertToResponse(payment);
    }

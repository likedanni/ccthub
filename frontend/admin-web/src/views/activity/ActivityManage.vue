            ...
            <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column label="操作" fixed="right" width="300">
                    <template #default="{ row }">
                        ...
                        <el-button v-if="row.auditStatus === 0" link type="success" size="small"
                            @click="handleAudit(row, 1)">通过</el-button>
                        <el-button v-if="row.auditStatus === 0" link type="danger" size="small"
                            @click="handleAudit(row, 2)">拒绝</el-button>
                        <el-button v-if="row.status === 0 && row.auditStatus === 1" link type="success" size="small"
                            @click="handleToggleStatus(row, 1)">上线</el-button>
                        <el-button v-if="row.status === 1" link type="warning" size="small"
                            @click="handleToggleStatus(row, 2)">下线</el-button>
        ...
        <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px">
            <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
                ...
                <el-form-item label="奖励配置" prop="rewardConfig">
                    <el-input v-model="form.rewardConfig" type="textarea" :rows="3"
                        placeholder='JSON格式，如：{"type":"points","value":100}' />
                </el-form-item>
            </el-form>
            <template #footer>
...
<script setup>
import { auditActivity, createActivity, deleteActivity, getActivityList, toggleActivityStatus, updateActivity } from '@/api/activity'
